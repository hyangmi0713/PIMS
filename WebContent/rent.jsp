			...
			<td>
				<div class="container" style="padding-top:30px; ">
					<table width="98%" class="type10" style="font-size:10pt;">
						<thead>
							<tr>
								...
								<th width="150" style="text-align:center;">대여수량(Pack)</th>
						...
						<tbody>
							<tr>
								<td>
									<select name="size" id="size" class="form-control" onchange="sizeChange(this)">
								...
								<td>
									<select name="kind" id="kind" class="form-control" onchange="kindChange(this)">
										<option value="none"></option>
									</select>
								</td>
								<td>
					...
					<table width="98%" class="type10" style="font-size:10pt;">
						<thead>
							<tr>
								<th width="150" style="text-align:center; background: #8ca6db;">인수자부서</th>
								<th width="150" style="text-align:center; background: #8ca6db;">인수자프로젝트</th>
								<th style="text-align:center; background: #8ca6db;">인수자이름</th>
								<th width="150" style="text-align:center; background: #b7a3eb;">인계자부서</th>
								<th width="150" style="text-align:center; background: #b7a3eb;">인계자프로젝트</th>
								<th style="text-align:center; background: #b7a3eb;">인계자이름</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>
									<select name="rentPart" id="rentPart" class="form-control" onchange="rentPartChange(this)">
										<option></option>
										<%
								...
								<td>
									<select name="rentProjectSel" id="rentProjectSel" class="form-control" >
										<option></option>
										<%
								...
								</td>
								<td>
									<input type="text" id="rentName" class="form-control">
								</td>
								<td>
									<select name="lendPart" id="lendPart" class="form-control" onchange="lendPartChange(this)">
										<option></option>
										<%
										for(int i=0; i<getPartList.size(); i++) {
										%>
								...
								<td>
									<select name="lendProjectSel" id="lendProjectSel" class="form-control" >
										<option></option>
										<%
										for(int i=0; i<getMyProjectDAO.size(); i++) {
										%>
					...
					<table width="98%" class="type10" id="dynamicTable" style="font-size:10pt;">
						<thead>
							<tr>
								...
								<th style="text-align:center;">대여수량(Pack)</th>
								<th style="text-align:center; background: #8ca6db;">인수자부서</th>
								<th style="text-align:center; background: #8ca6db;">인수자프로젝트</th>
								<th style="text-align:center; background: #8ca6db;">인수자이름</th>
								<th style="text-align:center; background: #b7a3eb;">인계자부서</th>
								<th style="text-align:center; background: #b7a3eb;">인계자프로젝트</th>
								<th style="text-align:center; background: #b7a3eb;">인계자이름</th>
								<th style="text-align:center;">비고</th>
							</tr>
						</thead>
					...
					<input type="button" class="btn btn=primary pull-right" value="다음" onclick="inputCreate()" style="width: 100px;">
				</div>
				<br><br>
			</td>
...
<script>
	...
	function tableCreate() {
		if (document.getElementById("size").value == "") {
			...
			alert("신청 수량을 입력해주세요 !");
		} else if (document.getElementById("size" && "kind" && "gram"
				&& "count").value != "") {
			...
			var rentPart = document.getElementById("rentPart").value;
			var rentProjectSel = document.getElementById("rentProjectSel").value;
			var rentName = document.getElementById("rentName").value;
			
			var lendPart = document.getElementById("lendPart").value;
			var lendProjectSel = document.getElementById("lendProjectSel").value;
			var lendName = document.getElementById("lendName").value;
			
			var rentProjectSplit = rentProjectSel.split(",");			
			var rentProjectNo = rentProjectSplit[0];
			var rentProjectName = rentProjectSplit[1];
			
			var lendProjectSplit = lendProjectSel.split(",");			
			var lendProjectNo = lendProjectSplit[0];
			var lendProjectName = lendProjectSplit[1];
			...
			html += '<td style="text-align:center;">' + rentPart + '</td>';
			html += '<td style="text-align:center;">' + rentProjectName + '</td>';
			html += '<td style="text-align:center;">' + rentName + '</td>';
			
			html += '<td style="text-align:center;">' + lendPart + '</td>';
			html += '<td style="text-align:center;">' + lendProjectName + '</td>';
			html += '<td style="text-align:center;">' + lendName + '</td>';
			...
			document.getElementById("rentPart").value = "";
			document.getElementById("rentProjectSel").value = "";
			document.getElementById("rentName").value = "";
			document.getElementById("lendPart").value = "";
			document.getElementById("lendProjectSel").value = "";
			...
			expaper_sub[5] = rentPart;
			expaper_sub[6] = rentProjectNo;
			expaper_sub[7] = rentProjectName;
			expaper_sub[8] = rentName;
			
			expaper_sub[9] = lendPart;
			expaper_sub[10] = lendProjectNo;
			expaper_sub[11] = lendProjectName;
			expaper_sub[12] = lendName;
	...
	function inputCreate() {
		...
		inputCreateForm.action = "rentAction.jsp";
...
var selSize = null;
	...
	function rentPartChange(e) {		
		var projectOption = [];
		var target = document.getElementById("rentProjectSel");
	...
	function lendPartChange(e) {		
		var projectOption = [];
		var target = document.getElementById("lendProjectSel");
